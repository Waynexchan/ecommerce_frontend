...
function ProductDetail() {
    ...
    const [specifications, setSepecifications] = useState([])
    ...
    useEffect(() => {
        apiInstance.get(`products/${param.slug}/`) //to grab the parameter and put into the url
            .then((res) => {
                setProduct(res.data);
                setSepecifications(res.data.specification)
                setGallery(res.data.gallery)
                setSize(res.data.size)
                setColor(res.data.color)
                if (res.data.color && res.data.color.length > 0) {
                    setSelectedColor(res.data.color[0]); // set the first color as default
                }
                if (res.data.size && res.data.size.length > 0) {
                    setSelectedSize(res.data.size[0]); // set the first size as default
                }
                        ...
                        <div>
                            ...
                            <form action="">
                                <div className="row flex-column">
                                    ...
                                    <div className="col-md-6 mb-4">
                                            <div className="form-outline">
                                                <label className="form-label" htmlFor="typeNumber"><b>Size:</b> {selectedSize?.name}</label>
                                            </div>
                                    ...
                                    <div className="col-md-6 mb-4">
                                            <div className="form-outline">
                                                <label className="form-label" htmlFor="typeNumber"><b>Color:</b> <span>{selectedColor.name}</span></label>
                                            </div>
                                            <div className='d-flex'>
                                                {color.map((c, index) => (
                                                    <div key={index}>
