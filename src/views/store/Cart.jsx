...
function Cart() {
  ...
  useEffect(() => {
    const source = axios.CancelToken.source(); // Add this line
    const fetchCartData = (cartId, userId) => {
      const url = userId ? `cart-list/${cartId}/${userId}` : `cart-list/${cartId}/`;
      apiInstance.get(url, { cancelToken: source.token }).then((res) => {
  ...
  const handleQuantityChange = useCallback((event, product_id) => {
  ...
  const updateCart = useCallback(async (product_id, price, shipping_amount, color, size) => {
    ...
    try {
      ...
      fetchCartData(cart_id, userData ? userData.user_id : null);
      fetchCartTotal(cart_id, userData ? userData.user_id : null);
  ...
  }, [productQuantities, userData, currentAddress, cart_id]);
